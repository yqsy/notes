...
节点号放入到`路由表数组`是有规则的.在0 <= i < 160的情况下,`路由表数组`的大小为160,对于不同的i下标的元素, 其中存储了`[2^i, 2^(i+1) )`距离范围的`节点数组`. 不同距离范围的k-bucket,容纳的节点数量的公式为`2^i`.
...
由上可见,`距离范围越小,最长公共祖先长度越大`.可以推出公式,最长公共祖先长度为`(bit len - i) - 1`.  由最长公共祖先公式推出i: `bit len - 1 - LCP` .
